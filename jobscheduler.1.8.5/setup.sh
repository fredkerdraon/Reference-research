...
then
  echo "${JAVABINPATH}java -Dizpack.mode=privileged -jar \"`dirname $0`/jobscheduler_linux-x86.1.8.5.jar\" $*"
  ${JAVABINPATH}java -Dizpack.mode=privileged -jar "`dirname $0`/jobscheduler_linux-x86.1.8.5.jar" $*
  exit 0
fi
...
if [ $sudo_exit -eq 0 ]
then
  my_sudo ${JAVABINPATH}java -jar "`dirname $0`/jobscheduler_linux-x86.1.8.5.jar" $*
  if [ $sudo_exit -ne 0 ]
  then
...
if [ "$USESU" = "y" ] || [ "$USESU" = "Y" ]
then
  echo "su root -c \"${JAVABINPATH}java -jar \\\"`dirname $0`/jobscheduler_linux-x86.1.8.5.jar\\\" $*\""
  su root -c "${JAVABINPATH}java -jar \"`dirname $0`/jobscheduler_linux-x86.1.8.5.jar\" $*"
fi